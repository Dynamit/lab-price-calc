...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function displayTestDetails(testCode) {
        ...
        if (details) {
            testDetailsContentDiv.innerHTML = `
                ...
                <p><strong>מבחנות נדרשות (M):</strong> ${details.tubes || "לא צוין"}</p>
                <p><strong>תנאי לקיחה ושימור (N):</strong> ${details.sampling_conditions || "לא צוין"}</p>
                <p><strong>הוראות שינוע (O):</strong> ${details.transport_instructions || "לא צוין"}</p>
                <p><strong>מידע על זמן ביצוע (P):</strong> ${details.execution_time_info || "לא צוין"}</p>
                <p><strong>משך זמן לקבלת תשובה (Q):</strong> ${details.results_time || "לא צוין"}</p>
            `;
        } else {
    ...
    function generatePdfViaPrint() {
        ...
        const quoteHtml = `
            ...
            <head>
                ...
                <style>
                    ...
                    h1, h2 { text-align: center; color: #007bff; margin-bottom: 15px; }
                    h2 { margin-top: 25px; }
                    p { margin: 5px 0; }
        ...
        setTimeout(() => {
            printWindow.print();
            // printWindow.close(); // Optional: close window after print dialog is actioned or closed
        }, 250);
    }
