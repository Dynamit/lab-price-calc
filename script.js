...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function generateQuotePdfViaPrint() {
        ...
        const finalAmountForVat = parseFloat(finalPriceSpan.textContent.replace(/[^\d.-]/g, ""));
        const vatAmount = Math.round(finalAmountForVat - (finalAmountForVat / 1.18));
        const nurseDiscountVal = nurseVisitDiscountInput ? parseFloat(nurseVisitDiscountInput.value) : 0;
        const nurseDiscountPercent = (nurseDiscountContainer && !nurseDiscountContainer.classList.contains("hidden") && nurseDiscountVal > 0) ? nurseDiscountVal : 0;
        const labTestsDiscountVal = labTestsDiscountInput ? parseFloat(labTestsDiscountInput.value) : 0;
        const labTestsDiscountPercent = (labTestsDiscountContainer && !labTestsDiscountContainer.classList.contains("hidden") && labTestsDiscountVal > 0) ? labTestsDiscountVal : 0;
        ...
        const quoteHtml = `
            ...
            <head>
                ...
                <style>
                    ...
                    .pdf-header img { max-height: 70px; margin-bottom: 10px; }
                    ...
                    .vat-info { font-size: 0.9em; text-align: right; margin-top: 5px; }
                </style>
            </head>
            <body>
                ...
                <table class="summary-table">
                    ...
                    <tr><td class="total-final"><strong>סכום סופי לתשלום (כולל מע"מ):</strong></td><td class="total-final"><strong>${finalPriceSpan.textContent.match(/\d+/g).join("")} ש\"ח</strong></td></tr>
                    <tr><td class="vat-info">מתוכו מע"מ (18%):</td><td class="vat-info"> ${formatPrice(vatAmount)} ש\"ח</td></tr>
                </table>
            </body>
            </html>
        `;
