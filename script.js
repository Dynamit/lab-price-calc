...
document.addEventListener("DOMContentLoaded", async () => {
    ...
    function updateCalculations() {
        ...
        if (finalPriceSpan) finalPriceSpan.innerHTML = overallDiscountPercent > 0 ? 
            `סכום סופי לתשלום (כולל ${overallDiscountPercent}% הנחה): <span id=\"finalAmount\">${formatPrice(finalPrice)}</span> ש\"ח` : 
            `סכום סופי לתשלום: <span id=\"finalAmount\">${formatPrice(finalPrice)}</span> ש\"ח`;
    }
    ...
    function generateQuotePdfViaPrint() {
        ...
        const finalAmountWithVat = parseFloat(finalPriceSpan.textContent.replace(/[^\d.-]/g, ""));
        const subtotalBeforeVat = Math.round(finalAmountWithVat / 1.18);
        const vatComponent = Math.round(finalAmountWithVat - subtotalBeforeVat);
        ...
        const quoteHtml = `
            ...
            <head>
                ...
                <style>
                    ...
                    .pdf-header img { max-height: 140px; margin-bottom: 10px; } /* Increased logo size */
            ...
            <body>
                <div class="pdf-header">
                    <img src="${logoSrc}" alt="לוגו החברה">
                ...
                <div class="section-summary">
                    <p>מחיר בסיס: ${formatPrice(currentNursePrice)} ש\"ח</p>
                    ${nurseDiscountPercent > 0 ? `<p>% הנחה חבילת בסיס: ${nurseDiscountPercent}%</p>` : ""}
                    <p><strong>מחיר חבילת בסיס ${nurseDiscountPercent > 0 ? "לאחר הנחה" : ""}: ${summaryNursePriceSpan.textContent} ש\"ח</strong></p>
                </div>
                ...
                <div class="section-summary">
                    <p>סכום ביניים לבדיקות ${labTestsDiscountPercent > 0 ? "(לפני הנחה)" : ""}: ${labTestsSubtotalSpan.textContent} ש\"ח</p>
                    ${labTestsDiscountPercent > 0 ? `<p>% הנחה בדיקות מעבדה: ${labTestsDiscountPercent}%</p>` : ""}
                    <p><strong>סה\"כ לבדיקות ${labTestsDiscountPercent > 0 ? "לאחר הנחה" : ""}: ${summaryLabTestsPriceSpan.textContent} ש\"ח</strong></p>
                </div>

                <h2>סיכום כללי</h2>
                <table class="summary-table">
                    <tr><td>חבילת בסיס ${nurseDiscountPercent > 0 ? "(לאחר הנחה)" : ""}:</td><td>${summaryNursePriceSpan.textContent} ש\"ח</td></tr>
                    <tr><td>סה\"כ בדיקות מעבדה ${labTestsDiscountPercent > 0 ? "(לאחר הנחה)" : ""}:</td><td>${summaryLabTestsPriceSpan.textContent} ש\"ח</td></tr>
                    <tr><td><strong>סה\"כ ביניים ${overallDiscountPercent > 0 ? "(לפני הנחה כללית)" : ""}:</strong></td><td><strong>${grandTotalSpan.textContent} ש\"ח</strong></td></tr>
                    ${overallDiscountPercent > 0 ? `<tr><td>% הנחה כללי:</td><td>${overallDiscountPercent}%</td></tr>` : ""}
                    <tr><td>סכום לפני מע"מ:</td><td>${formatPrice(subtotalBeforeVat)} ש\"ח</td></tr>
                    <tr><td>מע"מ (18%):</td><td>${formatPrice(vatComponent)} ש\"ח</td></tr>
                    <tr><td class="total-final"><strong>סכום סופי לתשלום (כולל מע"מ):</strong></td><td class="total-final"><strong>${formatPrice(finalAmountWithVat)} ש\"ח</strong></td></tr>
                </table>
            </body>
            </html>
        `;
